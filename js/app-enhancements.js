...
function addFavoriteStarsToResults(selectedFish) {
    ...
    selectedFish.forEach(fishKey => {
        ...
        const links = resultsDiv.querySelectorAll(`a.fish-name-link[href="species.html?fish=${fishKey}"]`);
        links.forEach(link => {
            if (link.nextSibling && link.nextSibling.classList && link.nextSibling.classList.contains('favorite-star')) {

...
const fishCategories = {
    "Tetras & Characins": ["neonTetra", "cardinalTetra", "greenNeonTetra", "rummyNoseTetra", "emberTetra", "glowlightTetra", "blackSkirtTetra", "blackNeonTetra", "buenosAiresTetra", "bloodfinTetra", "congoTetra", "lemonTetra", "serpaeTetra", "diamondTetra", "xRayTetra", "penguinTetra", "silverDollar", "emperorTetra", "flameTetra", "pristellaTetra", "blackPhantomTetra", "redPhantomTetra"],
    "Barbs": ["cherryBarb", "tigerBarb", "rosyBarb", "goldBarb", "odessaBarb", "denisonBarb", "fiveBandedBarb", "blackRubyBarb", "tictoBarb", "checkerBarb", "pentazonaBarb"],
    "Danios": ["zebraDanio", "pearlDanio", "celestialPearlDanio", "emeraldDwarfRasbora"],
    "Rasboras": ["harlequinRasbora", "chiliRasbora", "exclamationPointRasbora", "microrasbora", "scissortailRasbora", "lambchopRasbora", "brilliantRasbora", "phoenixRasbora", "strawberryRasbora"],
    "Rainbowfish": ["boesemansRainbowfish", "dwarfNeonRainbowfish", "turquoiseRainbowfish", "redRainbowfish", "threadfinRainbowfish"],
    "Minnows": ["whiteCloudMinnow", "goldWhiteCloud"],
    "Livebearers": ["guppy", "molly", "balloonMolly", "blackMolly", "platy", "variatusPlaty", "swordtail", "endlersLivebearer", "leastKillifish", "sailfinMolly", "dalmatianMolly", "mosquitofish"],
    "Bettas & Labyrinth Fish": ["bettaFish", "bettaImbellis", "paradiseFish"],
    "Gouramis": ["dwarfGourami", "honeyGourami", "pearlGourami", "sparklingGourami", "threeSpotGourami", "chocolateGourami", "opalineGourami", "goldGourami", "blueGourami", "kissingGourami", "moonlightGourami"],
    "Cichlids": ["angelfish", "germanBlueRam", "bolivianRam", "kribensis", "cockatooDwarfCichlid", "electricBlueAcara", "firemouthCichlid", "convictCichlid", "africanButterflyCichlid", "discus", "apistogrammaAgassizii", "blueAcara", "severum", "oscar", "jackDempsey", "jewelCichlid", "peacockCichlid", "yellowLabCichlid", "keyholeCichlid"],
    "Corydoras Catfish": ["corydoras", "pandaCory", "bronzeCory", "pepperedCory", "sterbaiCory", "juliiCory", "albinoCory", "pygmyCory", "saltAndPepperCory", "banditCory"],
    "Loaches": ["kuhliLoach", "yoyoLoach", "dwarfChainLoach", "zebraLoach", "clownLoach", "hillstreamLoach", "dojoLoach"],
    "Plecos & Algae Eaters": ["otocinclus", "bristlenosePleco", "clownPleco", "commonPleco", "rubberLipPleco", "chineseAlgaeEater", "siameseAlgaeEater"],
    "Other Catfish": ["stripedRaphaelCatfish", "glassCatfish", "upsideDownCatfish", "pictusCatfish", "twigCatfish", "dwarfPetricola"],
    "Oddball Fish": ["africanButterflyFish", "senegalBichir", "peaPuffer", "elephantNoseFish", "ropeFish"],
...
function analyzeCompatibility(fishData) {
    ...
    if (overlapMin <= overlapMax) {
        ...
        if (range >= 3) {
            ...
            warnings.push(`Narrow temperature overlap: ${overlapMin}-${overlapMax}°F. Monitor carefully.`);
        }
    } else {
        issues.push('No overlapping temperature range - these species require incompatible temperatures');
    }
    ...
    fishData.forEach((predator, i) => {
        fishData.forEach((prey, j) => {
            if (i !== j) {
                ...
                const isPredatory = predator.diet.toLowerCase().includes('carnivore') || 
                                  predator.diet.toLowerCase().includes('omnivore');
                
                // Shrimp/snails with fish
                if (prey.commonName.toLowerCase().includes('shrimp') || 
                    prey.commonName.toLowerCase().includes('snail')) {
                    if (predator.maxSize >= 2.0 && isPredatory) {
                        issues.push(`${predator.commonName} will likely eat ${prey.commonName} - invertebrates are seen as food by most fish`);
                    }
                }
                // Tiny fish (<1 inch) with larger fish
                else if (prey.maxSize < 1.0 && predator.maxSize >= 2.0 && isPredatory) {
                    issues.push(`${predator.commonName} (${predator.maxSize}") may prey on ${prey.commonName} (${prey.maxSize}") - size mismatch`);
                }
                // General size mismatch (3x+ ratio)
                else if (sizeRatio >= 3 && isPredatory) {
                    warnings.push(`${predator.commonName} (${predator.maxSize}") could prey on ${prey.commonName} (${prey.maxSize}") if hungry - monitor carefully`);
                }
            }
        });
    });
    ...
    const hasFullyAggressive = aggressionLevels.some(a => 
    ...
    const hasLongFinnedFish = fishNames.some(n => 
        n.includes('betta') || n.includes('angelfish') || n.includes('gourami')
    );
    
    if (hasBetta && (hasTigerBarb || hasRainbowShark || hasParadiseFish || hasBuenosAiresTetra || hasSerpaeTetra)) {
        issues.push('Bettas should NOT be kept with Tiger Barbs, Paradise Fish, Serpae Tetras, Buenos Aires Tetras, or Rainbow Sharks - extreme aggression and fin nipping likely');
    }
    
    if (hasParadiseFish && aggressionLevels.length > 1) {
        issues.push('Paradise Fish are AGGRESSIVE and will attack tankmates, especially long-finned species - species tank recommended');
    }
    
    if (hasConvictCichlid) {
        warnings.push('Convict Cichlids have EXTREME breeding aggression - will attack fish several times their size when protecting eggs/fry');
    }
    
    if ((hasTigerBarb || hasBuenosAiresTetra || hasSerpaeTetra) && hasLongFinnedFish) {
        warnings.push('Fin-nipping species present (Tiger Barb/Buenos Aires Tetra/Serpae Tetra) with long-finned fish - harassment likely');
    }
    ...
    if (hasChineseAlgaeEater) {
        warnings.push('Chinese Algae Eaters become increasingly aggressive with age - will latch onto flat-bodied fish (discus, angelfish)');
    }
    
    if (hasCommonPleco) {
        warnings.push('Common Plecos grow 18-24 inches and require 75-180+ gallon tanks - often mis-sold to beginners');
    }
    
    if (hasClownLoach) {
        warnings.push('Clown Loaches grow 10-12+ inches, live 20+ years, and need 125+ gallon tanks - long-term commitment required');
    }
    ...
    if (hasRainbowShark && aggressionLevels.length > 1) {
        ...
        if (otherFish.length > 0) {
            warnings.push('Rainbow Sharks are highly territorial and become increasingly aggressive with age - conflicts likely');
        }
    }
    ...
    if (hasFullyAggressive && hasPeaceful) {
        ...
        warnings.push('Mixing aggressive and semi-aggressive species - monitor closely for territorial disputes');
    } else if (hasSemiAggressive && hasPeaceful) {
        warnings.push('Semi-aggressive species with peaceful tank mates - provide hiding spots and monitor behavior');
    } else if (hasSemiAggressive && !hasPeaceful) {
        warnings.push('Multiple semi-aggressive species - establish territories with decorations and monitor for conflicts');
